...
%altneratief: maak gebruik van de echte snelheden
%v_x_2 = [15.3e-6*24*3600,16.3e-6*24*3600];

alpha=[0.001 0.067];
alpha_x=[0.5 0.5*alpha(2)/alpha(1)]; %m
% Opm: 500 keer groter dan in de paper, vanwege beperkte opslag voor 3D
% matrix...
%Mijn idee hier: analoog aan wat we hierboven deden met de snelheden: pas
%laag 2 aan obv de verhoudingen! 
D_x_2=alpha_x.*v_x_2;
...
Pe=0.8; %zie opm
deltax=Pe*min(alpha_x); %kleinere van de 2 alpha_x gebruiken
deltay=deltax;
deltaz=deltax;
Cr=0.8; %zie opm
...
difference = 1;
phi = 1e-3; %convergentiecriterium, verhogen voor sneller convergentie?
iter = 0;
