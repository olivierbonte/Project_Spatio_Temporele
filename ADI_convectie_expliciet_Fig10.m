...
% Coefficienten voor impliciete y-afgeleide: onveranderd!
...
tic
t_vis = 0:5:800; 
C_visualisatie = zeros(N+2,M+2,length(t_vis));
kvis = 1;
for tijd = t
    step=step+1;
    for j=1:N+2
        for i=1:M+2
            if j==1 %niet meer steunend op C(j-1,i)
                if i == 1
                    RL1(i)=(R/deltat-D_T*2/deltay^2)*C(j,i)+2*D_T/deltay^2*C(j+1,i) -...
                        v/(2*deltax)*C(j,i+1)+v/(2*deltax)*C(j,i+1);
                elseif i == M+2
                    RL1(i)=(R/deltat-D_T*2/deltay^2)*C(j,i)+2*D_T/deltay^2*C(j+1,i) -...
                        v/(2*deltax)*C(j,i-1)+v/(2*deltax)*C(j,i-1);
                else
            ...
            elseif j==N+2  %niet meer steunend op C(j+1,i)
                if i == 1
                    RL1(i)=2*D_T/deltay^2*C(j-1,i)+(R/deltat-D_T*2/deltay^2)*C(j,i) -...
                        v/(2*deltax)*C(j,i+1)+v/(2*deltax)*C(j,i+1);
                elseif i == M+2
                    RL1(i)=2*D_T/deltay^2*C(j-1,i)+(R/deltat-D_T*2/deltay^2)*C(j,i) -...
                        v/(2*deltax)*C(j,i-1)+v/(2*deltax)*C(j,i-1);
                ...
                if i == 1
                    RL1(i)=D_T/deltay^2*C(j-1,i)+(R/deltat-D_T*2/deltay^2)*C(j,i)+...
                        D_T/deltay^2*C(j+1,i) - v/(2*deltax)*C(j,i+1)+v/(2*deltax)*C(j,i+1);
                elseif i == M+2
                    RL1(i)=D_T/deltay^2*C(j-1,i)+(R/deltat-D_T*2/deltay^2)*C(j,i)+...
                        D_T/deltay^2*C(j+1,i) - v/(2*deltax)*C(j,i-1)+v/(2*deltax)*C(j,i-1);
                else
    ...
    %sjoemlen: zet manueel getallen kleiner dan 0 gelijk aan 0
    %id_replace = C(:,:) < 1e-5; %Dus zet alle kleine getallen gelijk aan al
    %C(id_replace) = 0;
    for i=1:M+2
        for j=1:N+2
            if i==1
    ...
    if rem(tijd,5) == 0
        ...
        ylabel('y')
        zlabel('C')
        title(strcat('ADI, convection explicit: time = ',num2str(tijd)))
        Ani(step+1) = getframe;
    end
...
sgtitle(strcat('ADI convection explicit, \Deltat = ',num2str(deltat),' days'))
exportgraphics(gcf,'Figuur_10_ADI_conv_explicit.png','Resolution',900)
toc
...
%figure()
%for i = 1:length(t_vis)
%    t = t_vis(i);
%    contourf(x,y,C_visualisatie(:,:,i))
%    colorbar
%    xlabel('x')
%    ylabel('y')
%    zlabel('C')
%    title(strcat('ADI, convection explicit: time = ',num2str(t)))
%    Ani(step+1) = getframe;
%end
%movie(Ani,0)
